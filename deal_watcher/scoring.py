...
def _absolute_value_score(
    price: int,
    specs: LaptopSpecs,
    config: DealWatcherConfig,
) -> tuple[int, int | None]:
    """Secondary absolute-price signal once a market baseline exists."""
    if not specs.gpu:
        return 0, None
...
def _cold_start_value_score(
    price: int,
    specs: LaptopSpecs,
    config: DealWatcherConfig,
) -> tuple[int, int | None]:
    """Primary value signal before enough MARKET observations exist.

    The configured GPU thresholds are intended to be actionable purchase
    anchors, not tiny bonuses.  Without this branch notify_score=80 was nearly
    unreachable during cold start even for obviously attractive listings.
    """
    if not specs.gpu:
        return 0, None
    threshold = config.gpu_thresholds.get(specs.gpu)
...
def _hardware_score(specs: LaptopSpecs) -> int:
    points = (
        5
        if specs.gpu in {"RTX 4080", "RTX 5070", "RTX 4070", "RTX 5060"}
        else (2 if specs.gpu else 0)
    )
...
def analyze_deal(
    *,
    ...
    if market.median_price:
        ...
        value_points = market_points + absolute_points
        cold_start = False
    else:
        value_points, threshold = _cold_start_value_score(price, specs, config)
        cold_start = True
    ...
    score = (
        ...
        + value_points
        + series_points
        + _CONDITION_POINTS[specs.condition]
        + _hardware_score(specs)
    )
    ...
    if discount_pct is not None:
        reasons.append(f"{discount_pct:+.1f}% к медиане рынка")
    elif cold_start and threshold:
        reasons.append("cold-start: рыночной выборки пока недостаточно")
    if threshold:
        reasons.append(f"ориентир {specs.gpu}: {threshold:,} ₽".replace(",", " "))
    if series_points:
        reasons.append(f"серия {specs.family}: {series_points:+d}")
    if specs.condition == Condition.LIKE_NEW:
        reasons.append("почти новый: минимальное использование")
    if risk.flags:
        reasons.append("риски: " + ", ".join(risk.flags))
    if price_drop_pct:
        reasons.append(f"снижение цены: {price_drop_pct:.1f}%")
