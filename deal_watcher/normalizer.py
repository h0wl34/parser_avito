...
_GPU_RE = re.compile(r"\b(?:geforce\s*)?(?:rtx\s*[- ]?)?(?P<gpu>50[5-9]0|40[5-9]0|30[5-9]0)\b", re.I)
_CPU_PATTERNS = [
    re.compile(r"\b(?:ryzen\s*)?(?P<cpu>[3579]\s*\d{3,4}(?:HX|HS|H|U|X3D)?)\b", re.I),
...
_STORAGE_RE = re.compile(r"\b(?P<size>256|512|1024|2048|4096)\s*(?:gb|гб)\b|\b(?P<tb>[124])\s*(?:tb|тб)\b", re.I)
_SLASH_RE = re.compile(r"\b(?P<ram>16|24|32|48|64|96|128)\s*/\s*(?P<storage>512|1024|2048|[124])(?:\s*(?:tb|тб))?\b", re.I)

_BROKEN = ("не включается", "не работает", "на запчасти", "разбит", "кирпич", "дефект")
_REFURB = ("восстановлен", "refurb", "после ремонта", "ремонтирован", "замена платы")
_USED = ("б/у", "бу ", "бывший в употреблении", "пользовался", "пользовались", "есть следы использования")
_NEW_STRONG = ("не активирован", "неактивирован", "запечатан", "sealed", "не вскрыт")
_NEW_WEAK = ("новый", "new", "полный комплект", "гарантия")

_RISK_TERMS: list[tuple[str, int, str]] = [
    ("trade-in", 18, "trade-in"), ("trade in", 18, "trade-in"), ("трейд-ин", 18, "trade-in"),
    ("цена от", 14, "цена «от»"), ("уточняйте цену", 16, "цена требует уточнения"),
    ("уточняйте стоимость", 16, "цена требует уточнения"),
    ("при оформлении кредита", 24, "цена при кредите"), ("при покупке в кредит", 24, "цена при кредите"),
    ("первоначальный взнос", 35, "первоначальный взнос вместо цены"),
    ("под заказ", 12, "под заказ"), ("предоплата", 18, "предоплата"),
    ("цена указана за", 30, "цена может быть не за ноутбук"), ("без видеокарты", 50, "без видеокарты"),
    ("на запчасти", 50, "на запчасти"), ("не включается", 50, "не включается"),
    ("после ремонта", 22, "после ремонта"), ("восстановлен", 22, "восстановленный"),
...
def extract_specs(title: str | None, description: str | None = None) -> LaptopSpecs:
    ...
    family = next((name for pattern, name in _FAMILY_PATTERNS if pattern.search(raw)), None)
    ...
    return LaptopSpecs(brand=brand, family=family, sku=sku, cpu=cpu, gpu=gpu, ram_gb=ram_gb, storage_gb=storage_gb, condition=detect_condition(raw))
