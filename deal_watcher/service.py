...
class DealWatcherService:
    ...
    def analyze_item(self, ad: Any, source_url: str | None = None) -> DealAnalysis | None:
        ...
        price_drop_pct = self.store.record_listing(avito_id=avito_id, title=title, seller_id=getattr(ad, "sellerId", None), url=getattr(ad, "urlPath", None), price=price, specs=specs, source_url=source_url, published_at=published_at)
        market = self.store.market_stats(specs, window_days=self.config.market_window_days, min_samples=self.config.min_market_samples, exclude_avito_id=avito_id)
        return analyze_deal(price=price, specs=specs, risk=risk, market=market, config=self.config, price_drop_pct=price_drop_pct)
